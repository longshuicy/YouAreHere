...
A looking-at piece, not a solving piece. Thirty-one worlds, 2,251 characters and 10,565 ties,
presented for comparison: what a story looks like as a shape, and what a life looks like from
inside one.
...
One card per world, thirty-one of them, drawn identically at the same scale. Degree distribution,
concentration, modularity, horizon spread. Anonymising *worlds* rather than characters: far more
legible than a wall of ego networks, it needs nothing hidden, and it grows on its own as books are
added.
...
The Taming of the Shrew  Phillip  knows 8 → reaches 17   2.1   (three-way tie, all servants)
```
...
Do not plot raw two-hop reach: it saturates, with seventeen of thirty-one worlds averaging above
0.95, and only re-draws cast size. Do not plot the world average either — it tracks cast size
...
Coverage decides what it can honestly do:

| facet | characters | worlds |
|---|---|---|
| books, unit, corpusSize | 100% | 31 |
| gender | 90.8% | 31 |
| titles | 24.3% | 13 |
| occupation | 6.4% | 11 |
| houses, born, culture, died | 8–15% | 1 |
| pov, species, homeworld, affiliations | under 2% | 1 |

Search and filter work everywhere. Cross-corpus facet *ranking* only works on gender — still 2,044
characters across thirty-one worlds, which is enough to say something real. Everything below
`titles` is a room inside one world, not a claim about the corpus.

### 4 · Rarity

The remainder of the cross-universe look-alike idea, which does not survive as a leaderboard: the
difficulty signature occupies only 67 buckets for 2,251 characters, median fourteen to a bucket, so
"most look-alikes" is a 173-way tie for generic. The rare end is the exhibit. Exactly seven
characters are structurally unique corpus-wide — Emilia in *Othello*, Wyman Manderly, Tema and
three others from the Bible, the Duke of Buckingham.

Corpus-relative, so it recomputes on every addition and the list of seven will change. That is the
point of it, not a defect.
...
| Plain-against-weighted rank gap | who meets many people once, against who is actually present throughout | nothing new — this is the Shemaiah/Azariah observation in `_prominence`, promoted to a number |
...
**Relative** — rarity, look-alikes, percentiles. Defined against the corpus, so every one of them
changes when a book is added.

The game already has this problem. `build()` is a deliberate two-pass because `corpus_index` spans
every world, which means `ease` — baked into 1,257 puzzle records — goes stale the moment a book is
added unless everything is rebuilt, and `--source X` silently scores against a corpus of one.

So the gallery computes at load, in the client, and stores nothing. Nodes and edges ship; prominence
and signature derive from them in about fifteen lines. Two thousand nodes is nothing in a browser.
Adding a book then costs the gallery no work at all — the world appears and every figure re-derives
against the new whole.

Two notes. Community detection must be seeded or use deterministic tie-breaking, or it breaks the
build's promise that two runs on unchanged input produce an empty diff. And the metadata files
total 2.8 MB, mostly the 1,840 quoted lines; the gallery wants its own payload — names, facts,
...
Seeing thirty anonymous worlds before you have inhabited one teaches you to read them as data. The
game works because you read your own as a life first.

### Preconditions

1. The main loop is playable end to end on real data.
2. Someone who is not you has solved a puzzle without being told how.
3. At least three universes are loaded. Satisfied — thirty-one.
...
**Is there a thesis, or only an instrument?** Twenty-eight of the thirty-one worlds are Shakespeare
— 830 characters, one hand, thirty-five years, uniform extraction. If genre is recoverable from
topology alone, that is the argument the gallery is missing, and it costs a hand-labelled table of
twenty-eight rows to test.
