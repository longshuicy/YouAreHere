...
>
> Every node's menu with more than one drawn tie carries a free row: `SHARES MOST — Sansa Stark · 47`. Hovering it lights that tie in the accent on the paper, dims the rest of the fan, rings the far node and sets the figure beside the tie. Clicking follows it: the tie stays lit and the far node's own menu opens, so naming, expanding or reading them is the next click rather than a hunt for a circle.
>
...
>
> **The figure is a bare number.** No unit, ever — *47 shared scenes* against *47 verses naming both* would hand over the dataset, and the word *weight* stays out of the player's sight as it always has. Unitless it does the one job it is there for: 47 against 12 settles in a glance what two strokes a pixel apart never will. It is shown only on a tie the player has asked to see lit.
